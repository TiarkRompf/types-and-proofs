...
Definition env_kv J :=
  forall x K, indexr x J = Some K -> val_sort K.
...
Lemma aux0: forall i i' J K
  (eq: i = i')
  (h2: forall i K, indexr i (J) = Some K -> val_sort K)
  (h2A: indexr i (J) = Some K -> val_sort K)
  (h2B: indexr i' (J) = Some K -> val_sort K)
  (eq2A: h2A = h2 i K)
  (eq2B: h2B = h2 i' K)
  eA eB,
    h2A eA = h2B eB.
...
Proof.
  ...
  - dependent destruction Heq.
    ...
    remember (eq_ind (if length J =? length J then Some K1 else indexr (length J) J)
      (fun o : option tm => o = Some K1) e (Some K1) e).
    simpl in e0.
...
Proof.
  ...
  - (* TAll *)
    ...
    assert (forall VT1, val_type h1 (envkv_cons (J'++J) T1 VT1 h2) =
                          val_type (haskind_weaken1 T2 (T1::J') J TStar K1 h1) (envkv_weaken (T1::J') J K1 vk (envkv_cons (J'++J) T1 VT1 h2))). {
    intros.
    specialize IHT2 with (J':=T1::J') (J:=J) (vk:=vk) (h1:=h1) (h2:=(envkv_cons (J'++J) T1 VT1 h2)).
    eapply IHT2. }
    eapply functional_extensionality in H.
    rewrite H.
    assert (forall VT1,
     val_type (haskind_weaken1 T2 (T1 :: J') J TStar K1 h1) (envkv_weaken (T1 :: J') J K1 vk (envkv_cons (J' ++ J) T1 VT1 h2)) = val_type (haskind_weaken1 T2 (T1 :: J') J TStar K1 h1) (envkv_cons (J' ++ K1 :: J) T1 VT1 (envkv_weaken J' J K1 vk h2))).
...
Proof.
  ...
  - (* TVar *)
    ...
    + subst i.
      simpl in *.
      assert (HK1EQ: K1 = K). eapply xxx3; eauto.
      subst K1.
  ...
  - (*TAll*) dependent destruction h1f. simpl.
    ...
    assert (HV: forall VT1, val_type h1f (envkv_cons (J' ++ K1 :: J) T2_1 VT1 (envkv_weaken J' J K1 T1K WFJ)) =
      val_type (eq_rec_r (fun t : tm => has_kind (T2_1 :: J' ++ J) (subst T2_2 (length J) t) TStar) (hask_subst T2_2 (T2_1 :: J') J T1 K1 TStar h1f HK1) (splice_acc T1 (length J) (length J') 1))
      (envkv_cons (J' ++ J) T2_1 VT1 WFJ)). { intros.
      rewrite splice_acc. rewrite aux2.
      specialize (IHT2_2 (T2_1 :: J') J K1 TStar T1K (envkv_cons (J' ++ J) T2_1 VT1 WFJ) T1 h1f HK1).
  ...
  - (*TTAbs*) dependent destruction h1f. remember (k_tabs (J' ++ K1 :: J) T2_1 T2_2 K2 h1f) as h1f'.
    ...
    rewrite aux2.
    assert (HT1K: val_type (haskind_extend_mult (T2_1 :: J') T1 J K1 HK1) (envkv_cons (J' ++ J) T2_1 vk WFJ) = val_type (haskind_extend_mult J' T1 J K1 HK1) WFJ). { intros.
      simpl. elim_eq_rect. simpl. elim_eq_rect. simpl.
      rewrite <- valt_extend'. reflexivity.
    } rewrite <- HT1K in H.
...
Lemma valt_subst1: forall T2 J K1 T1K WFJ T1 (h1f : has_kind (K1 :: J) T2 TStar) HK1 vy,
...
Lemma envt_extendk: forall E G J h2 K1 (vk: val_sort K1),
    env_type E G J h2 ->
